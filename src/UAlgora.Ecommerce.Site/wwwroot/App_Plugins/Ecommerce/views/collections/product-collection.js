...
export class ProductCollection extends UmbElementMixin(LitElement) {
  ...
  _getEmptyProduct() {
    return { name: '', sku: '', description: '', shortDescription: '', price: 0, costPrice: 0, salePrice: null,
      currencyCode: 'USD', taxCategoryId: null, taxIncluded: false, categoryId: null, manufacturerId: null, brandId: null,
  ...
  async _loadCurrencies() { try { const r = await fetch('/umbraco/management/api/v1/ecommerce/currency', { credentials: 'include' }); if (r.ok) this._currencies = await r.json(); } catch (e) { this._currencies = [{code:'USD',name:'US Dollar'},{code:'EUR',name:'Euro'}]; } }
