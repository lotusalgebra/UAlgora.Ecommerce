...
export class CategoriesMenuItem extends UmbElementMixin(LitElement) {
  static styles = css`
    :host { display: block; }
    .menu-section { margin-bottom: var(--uui-size-space-4); }
    .menu-header {
      display: flex;
      align-items: center;
    ...
    .tree-container { margin-left: var(--uui-size-space-4); }
    .tree-item {
      display: flex;
      align-items: center;
    ...
    .tree-item:hover { background: var(--uui-color-surface-emphasis); }
    .tree-item uui-icon { margin-right: var(--uui-size-space-2); color: var(--uui-color-text-alt); }
    ...
    .loading { padding: var(--uui-size-space-4); text-align: center; }
  `;
  ...
  _handleNodeClick(node) {
    window.location.href = `/umbraco/section/ecommerce/workspace/ecommerce-category/edit/${node.id}`;
  }
          ...
          <div class="tree-container">
            ...
            ` : this._nodes.map(node => html`
              <div class="tree-item" @click=${() => this._handleNodeClick(node)}>
                ...
                ${node.productCount ? html`<span class="badge">${node.productCount}</span>` : ''}
              </div>
            `)}
          </div>
        ` : ''}
...
customElements.define('ecommerce-categories-menu-item', CategoriesMenuItem);
