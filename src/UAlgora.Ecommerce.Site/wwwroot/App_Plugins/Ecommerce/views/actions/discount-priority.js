...
/**
 * Discount Priority Action
 * Quick action to change discount priority (lower numbers = higher priority).
 */
export class DiscountPriorityAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  _loadFromWorkspace() {
    ...
    if (workspace) {
      ...
      this._priority = discount?.priority ?? 0;
    }
  }
  ...
  async _handlePriority() {
    ...
    const input = prompt(
      ...
      `Enter new priority (lower numbers = higher priority):`,
    ...
    try {
      const response = await fetch(`/umbraco/management/api/v1/ecommerce/discount/${discount.id}/update-priority`, {
        method: 'POST',
        headers: {
  ...
  render() {
    const isHighPriority = this._priority <= 1;
...
customElements.define('ecommerce-discount-priority-action', DiscountPriorityAction);

export default DiscountPriorityAction;
