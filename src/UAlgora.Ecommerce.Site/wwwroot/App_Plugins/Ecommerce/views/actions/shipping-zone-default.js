...
/**
 * Shipping Zone Set Default Action
 * Quick action to set a shipping zone as the default.
 */
export class ShippingZoneDefaultAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  async _handleSetDefault() {
    ...
    const confirmed = confirm(`Set "${zone.name}" as the default shipping zone? This will remove the default status from any other zone.`);
    if (!confirmed) return;

    this._processing = true;

    try {
      const response = await fetch(`/umbraco/management/api/v1/ecommerce/shipping/zone/${zone.id}/set-default`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const reloadResponse = await fetch(`/umbraco/management/api/v1/ecommerce/shipping/zone/${zone.id}`, {
...
customElements.define('ecommerce-shipping-zone-default-action', ShippingZoneDefaultAction);

export default ShippingZoneDefaultAction;
