...
/**
 * Shipping Method Sort Order Action
 * Quick action to change shipping method sort order.
 */
export class ShippingMethodSortAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  _loadFromWorkspace() {
    ...
    if (workspace) {
      ...
      this._sortOrder = method?.sortOrder ?? 0;
    }
  }
  ...
  async _handleSort() {
    ...
    try {
      const response = await fetch(`/umbraco/backoffice/ecommerce/shipping/method/${method.id}/update-sort`, {
        method: 'POST',
        headers: {
...
customElements.define('ecommerce-shipping-method-sort-action', ShippingMethodSortAction);

export default ShippingMethodSortAction;
