...
/**
 * Payment Gateway Sort Order Action
 * Quick action to change payment gateway sort order.
 */
export class PaymentGatewaySortAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  _loadFromWorkspace() {
    ...
    if (workspace) {
      ...
      this._sortOrder = gateway?.sortOrder ?? 0;
    }
  }
  ...
  async _handleSort() {
    ...
    try {
      const response = await fetch(`/umbraco/backoffice/ecommerce/payment/gateway/${gateway.id}/update-sort`, {
        method: 'POST',
        headers: {
...
customElements.define('ecommerce-payment-gateway-sort-action', PaymentGatewaySortAction);

export default PaymentGatewaySortAction;
