...
/**
 * Shipping Zone Sort Order Action
 * Quick action to change shipping zone sort order.
 */
export class ShippingZoneSortAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  async _handleSort() {
    ...
    try {
      const response = await fetch(`/umbraco/backoffice/ecommerce/shipping/zone/${zone.id}/update-sort`, {
        method: 'POST',
        headers: {
...
customElements.define('ecommerce-shipping-zone-sort-action', ShippingZoneSortAction);

export default ShippingZoneSortAction;
