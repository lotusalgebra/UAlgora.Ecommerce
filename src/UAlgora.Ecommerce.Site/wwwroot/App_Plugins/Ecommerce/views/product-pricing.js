...
export class ProductPricing extends UmbElementMixin(LitElement) {
  ...
  static properties = {
    ...
    _currencies: { type: Array, state: true },
    _taxCategories: { type: Array, state: true }
  };
  ...
  async _loadCurrencies() {
    try {
      const response = await fetch('/umbraco/management/api/v1/ecommerce/currency', {
        headers: { 'Accept': 'application/json' }
      });
      if (response.ok) {
        const data = await response.json();
        this._currencies = data.items || [];
      }
    } catch (error) {
  ...
  async _loadTaxCategories() {
    try {
      const response = await fetch('/umbraco/management/api/v1/ecommerce/tax-category', {
        headers: { 'Accept': 'application/json' }
      });
      if (response.ok) {
        const data = await response.json();
        this._taxCategories = data.items || [];
      }
    } catch (error) {
  ...
  _handleToggleInput(field, event) {
  ...
  _getBreakEvenQty() {
    const profit = this._getProfit();
    if (profit <= 0) return '∞';
    const fixedCosts = this._product?.fixedCosts || 0;
    if (fixedCosts === 0) return '1';
    return Math.ceil(fixedCosts / profit).toString();
  }

  _getPriceWithTax() {
    const effectivePrice = this._getEffectivePrice();
    if (this._product?.taxIncluded || this._product?.taxExempt) return effectivePrice;
    const taxCategory = this._taxCategories.find(t => t.id === this._product?.taxCategoryId);
    const taxRate = taxCategory?.rate || 0;
    return effectivePrice * (1 + taxRate / 100);
  }

  _getCurrencySymbol() {
    const currency = this._currencies.find(c => c.code === this._product?.currencyCode);
  ...
  render() {
    ...
    return html`
      ...
      <h3 class="section-title">Currency & Tax Settings</h3>
      <uui-box>
        <div class="form-grid">
          <div class="form-group">
            ...
            <select
              id="currencyCode"
              style="width: 100%; padding: 10px; border: 1px solid var(--uui-color-border); border-radius: var(--uui-border-radius);"
              .value=${this._product.currencyCode || 'USD'}
              @change=${(e) => this._handleSelectInput('currencyCode', e)}
            >
              ${this._currencies.map(c => html`
                <option value="${c.code}" ?selected=${this._product.currencyCode === c.code}>
                  ${c.symbol} ${c.code} - ${c.name}
                </option>
              `)}
            </select>
            <div class="hint">Currency for this product's prices</div>
          </div>

          <div class="form-group">
            <label for="taxCategoryId">Tax Category</label>
            <select
              id="taxCategoryId"
              style="width: 100%; padding: 10px; border: 1px solid var(--uui-color-border); border-radius: var(--uui-border-radius);"
              .value=${this._product.taxCategoryId || ''}
              @change=${(e) => this._handleSelectInput('taxCategoryId', e)}
            >
              <option value="">-- Select Tax Category --</option>
              ${this._taxCategories.map(t => html`
                <option value="${t.id}" ?selected=${this._product.taxCategoryId === t.id}>
                  ${t.name} (${t.rate}%)
                </option>
              `)}
            </select>
            <div class="hint">Tax category for tax calculation</div>
          </div>
          ...
          <div class="form-group">
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
              <input
                type="checkbox"
                id="taxIncluded"
                .checked=${this._product.taxIncluded || false}
                @change=${(e) => this._handleToggleInput('taxIncluded', e)}
                style="width: 18px; height: 18px;"
              />
              Prices include tax
            </label>
            <div class="hint">Enable if entered prices already include tax</div>
          </div>
          ...
          <div class="form-group">
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
              <input
                type="checkbox"
                id="taxExempt"
                .checked=${this._product.taxExempt || false}
                @change=${(e) => this._handleToggleInput('taxExempt', e)}
                style="width: 18px; height: 18px;"
              />
              Tax Exempt
            </label>
            <div class="hint">This product is exempt from tax</div>
          </div>
        </div>
      </uui-box>
      ...
      <uui-box>
        <div class="price-summary" style="grid-template-columns: repeat(4, 1fr);">
          <div class="price-card">
            <div class="price-card-label">ROI</div>
            <div class="price-card-value ${this._getROI() >= 100 ? 'positive' : 'warning'}">
              ${this._getROI().toFixed(1)}%
            </div>
          </div>
          <div class="price-card">
            <div class="price-card-label">Markup</div>
            <div class="price-card-value ${this._getMarkup() >= 50 ? 'positive' : 'warning'}">
              ${this._getMarkup().toFixed(1)}%
            </div>
          </div>
          <div class="price-card">
            <div class="price-card-label">Break-even Qty</div>
            <div class="price-card-value">
              ${this._getBreakEvenQty()}
            </div>
          </div>
          <div class="price-card">
            <div class="price-card-label">Price with Tax</div>
            <div class="price-card-value">
              ${this._getCurrencySymbol()}${this._getPriceWithTax().toFixed(2)}
            </div>
          </div>
        </div>

        <div style="margin-top: var(--uui-size-layout-1); padding: var(--uui-size-layout-1); background: var(--uui-color-surface-alt); border-radius: var(--uui-border-radius);">
          <h4 style="margin: 0 0 var(--uui-size-space-4) 0;">Pricing Recommendations</h4>
          <ul style="margin: 0; padding-left: var(--uui-size-layout-1); color: var(--uui-color-text-alt);">
            ${profitMargin < 20 ? html`<li style="color: var(--uui-color-warning);">Consider increasing price - profit margin is below 20%</li>` : ''}
            ${profitMargin >= 50 ? html`<li style="color: var(--uui-color-positive);">Excellent profit margin - room for promotional discounts</li>` : ''}
            ${this._product.compareAtPrice && this._product.basePrice >= this._product.compareAtPrice ? html`<li style="color: var(--uui-color-danger);">Compare at price should be higher than base price</li>` : ''}
            ${this._product.salePrice && this._product.salePrice >= this._product.basePrice ? html`<li style="color: var(--uui-color-danger);">Sale price should be lower than base price</li>` : ''}
            ${!this._product.costPrice ? html`<li>Add cost price for accurate profit calculations</li>` : ''}
            ${this._priceTiers.length === 0 ? html`<li>Consider adding tiered pricing for bulk orders</li>` : ''}
          </ul>
        </div>
      </uui-box>
    `;
