...
/**
 * Tax Zone Clear Regions Action
 * Quick action to clear all geographic regions from a zone.
 */
export class TaxZoneClearRegionsAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  async _handleClear() {
    ...
    try {
      const response = await fetch(`/umbraco/backoffice/ecommerce/tax/zone/${zone.id}/clear-regions`, {
        method: 'POST',
        headers: {
...
customElements.define('ecommerce-tax-zone-clear-regions-action', TaxZoneClearRegionsAction);

export default TaxZoneClearRegionsAction;
