...
/**
 * Tax Zone Priority Action
 * Quick action to change tax zone matching priority (higher = checked first).
 */
export class TaxZonePriorityAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  _loadFromWorkspace() {
    ...
    if (workspace) {
      ...
      this._priority = zone?.priority ?? 0;
    }
  }
  ...
  async _handlePriority() {
    ...
    const input = prompt(
      ...
      `Enter new priority (higher numbers are checked first when matching addresses):`,
    ...
    try {
      const response = await fetch(`/umbraco/backoffice/ecommerce/tax/zone/${zone.id}/update-priority`, {
        method: 'POST',
        headers: {
...
customElements.define('ecommerce-tax-zone-priority-action', TaxZonePriorityAction);

export default TaxZonePriorityAction;
