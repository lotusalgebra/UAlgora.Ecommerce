...
/**
 * Category Sort Order Action
 * Quick action to change category sort order.
 */
export class CategorySortAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  async _handleSort() {
    ...
    try {
      const response = await fetch(`/umbraco/management/api/v1/ecommerce/category/${category.id}/update-sort`, {
        method: 'POST',
        headers: {
...
customElements.define('ecommerce-category-sort-action', CategorySortAction);

export default CategorySortAction;
