...
/**
 * Payment Method Sort Order Action
 * Quick action to change payment method sort order.
 */
export class PaymentMethodSortAction extends UmbElementMixin(LitElement) {
  static styles = css`
    :host {
      display: contents;
    }
  ...
  async _handleSort() {
    ...
    try {
      const response = await fetch(`/umbraco/backoffice/ecommerce/payment/method/${method.id}/update-sort`, {
        method: 'POST',
        headers: {
...
customElements.define('ecommerce-payment-method-sort-action', PaymentMethodSortAction);

export default PaymentMethodSortAction;
